...
SGT = pytz.timezone('Asia/Singapore')  # Singapore Standard Time
...
def add_page_visited_details(pagename, timeOfvisit=None):
    if timeOfvisit is None:
        timeOfvisit = datetime.now(SGT).strftime("%Y-%m-%d %H:%M:%S")
    else:
        timeOfvisit = timeOfvisit.astimezone(SGT).strftime("%Y-%m-%d %H:%M:%S")
